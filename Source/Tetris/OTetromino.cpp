...
AOTetromino::AOTetromino()
{
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> mat(TEXT("MaterialInterface'/Game/Materials/mOTetromino.mOTetromino'"));
	FLinearColor matColor;
	if (mat.Succeeded())
	{
		UMaterialInstanceDynamic* instance = UMaterialInstanceDynamic::Create(mat.Object, NULL);
		//instance->GetLinearColorParameterValue(FName("Base Color"), matColor);
		thisMat = instance;
	...
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> flux(TEXT("/Game/Materials/FluxMaterials/mOFluxBlock.mOFluxBlock"));
...
void AOTetromino::BeginPlay()
{
	...
	spawningCoords[1] = FVector2D(0, -1);
	...
	rotStateOne[1] = FVector2D(1, 0);
	rotStateOne[2] = FVector2D(1, 1);
	rotStateOne[3] = FVector2D(0, 1);
	...
	rotStateThree[1] = FVector2D(-1, 0);
	rotStateThree[2] = FVector2D(-1, -1);
